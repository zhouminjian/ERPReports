		...
		if($datetype=='2'){
			for($i=1;$i<=3;$i++){//1：经销  2：加工 3:双经销
				...
				while(strcmp($tmpMon,$endMon)<=0){
					$sql = "select SUM(c.OrderQty)+(select SUM(c.OrderQty)*0.9144 
	from Dye.OrderColor c ,Dye.OrderMain m 
	where (SheetDate between '$startMon' and '$tmpMon') $sheetkind and c.SheetSeries=m.SheetSeries and SheetStatus='0' and Unit='2' $businessKind) QTY
	from Dye.OrderColor c ,Dye.OrderMain m 
	where (SheetDate between '$startMon' and '$tmpMon') $sheetkind and c.SheetSeries=m.SheetSeries and SheetStatus='0' and Unit='0' $businessKind";
					$result = sqlsrv_query($conn,$sql);
					while($row = sqlsrv_fetch_array($result, SQLSRV_FETCH_ASSOC)){
						//将结果集放到数组中
						if($i==1){
							array_push($xAxis,substr(date("Ymd",strtotime($startMon)),0,6));
							array_push($y1Axis,isset($row['QTY'])?$row['QTY']:0);
						}
						else if($i==2){
							array_push($y2Axis,isset($row['QTY'])?$row['QTY']:0);
						}
						else{
							array_push($y3Axis,isset($row['QTY'])?$row['QTY']:0);
						}
					}
			...
			for($i=1;$i<=3;$i++){
				...
				while(strcmp($tmpYear,$endYear)<=0){
					$sql = "select SUM(c.OrderQty)+(select SUM(c.OrderQty)*0.9144 
						from Dye.OrderColor c ,Dye.OrderMain m 
						where (SheetDate between '$startYear' and '$tmpYear') $sheetkind and c.SheetSeries=m.SheetSeries and SheetStatus='0' and Unit='2' $businessKind) QTY
						from Dye.OrderColor c ,Dye.OrderMain m 
						where (SheetDate between '$startYear' and '$tmpYear') $sheetkind and c.SheetSeries=m.SheetSeries and SheetStatus='0' and Unit='0' $businessKind";
					$result = sqlsrv_query($conn,$sql);
					while($row = sqlsrv_fetch_array($result, SQLSRV_FETCH_ASSOC)){
						//将结果集放到数组中
						if($i==1){
