...
select p.*,ROW_NUMBER() OVER(Order by SpecNum desc) as RowId from
	(select x.CustomerID,x.NameC,x.Spec,count(x.Spec)SpecNum from
		...
		where om.SheetDate between '$startYear' and '$endYear' and ct.CustomerID='$kh'
		group by ct.CustomerID,ct.NameC,oc.SheetSeries,om.Spec)x
	group by x.CustomerID,x.NameC,x.Spec)p)s ";
